      ...
      expose.to
        ? expose.to
            ...
            .flatMap(to => {
              ...
              const leasedEp = to.ip?.length > 0 ? { kind: Endpoint_LEASED_IP, sequence_number: endpointSequenceNumbers[to.ip] || 0 } : undefined;
  ...
  v3Groups() {
    ...
    for (const [svcName, service] of services) {
      for (const [placementName, svcdepl] of Object.entries(this.data.deployment[svcName])) {
        ...
        if (!group) {
          const attributes = (infra.attributes ? Object.entries(infra.attributes).map(([key, value]) => ({ key, value })) : []) as unknown as Array<{
            key: string;
            value: string;
          }>;
      ...
      Object.keys(depl).forEach(placementName => {
        ...
        if (!group) {
          group = {
            name: placementName,
            requirements: {
              attributes: infra.attributes ? Object.entries(infra.attributes).map(([key, value]) => ({ key, value })) : [],
              signedBy: {
                allOf: infra.signedBy?.allOf || [],
                anyOf: infra.signedBy?.anyOf || []
              }
